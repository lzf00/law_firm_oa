...
<template>
  <section class="module-page">
    <div class="page-intro">
      <div><span class="eyebrow">ORGANIZATION · {{ people.length }} PEOPLE</span><h2>{{ t('headline.directory') }}</h2><p>通讯录与钉钉组织身份可同步，展示部门、岗位角色、脱敏联系方式和账号状态。</p></div>
    </div>
    <div class="directory-layout">
      <aside class="panel department-panel">
        <div class="panel-heading"><div><span class="eyebrow">DEPARTMENTS</span><h3>组织架构</h3></div><Building2 :size="20" /></div>
      ...
      <div>
        <div class="toolbar"><label class="search-box wide"><Search :size="16" /><input v-model="keyword" placeholder="搜索姓名、部门、角色或邮箱" /></label></div>
        <div class="directory-grid">
          <article v-for="person in filtered" :key="person.id" class="person-card">
            ...
            <div><h3>{{ person.displayName }}</h3><span>{{ (locale === 'en-US' ? person.officeNameEn : person.officeNameZh) || person.departments || '未分配部门' }}</span><small>{{ person.roles || '普通成员' }} · {{ person.timezone || '—' }}</small></div>
            <a v-if="person.email" :href="`mailto:${person.email}`" :aria-label="`邮件联系${person.displayName}`"><Mail :size="16" /></a>
          </article>
          <div v-if="!filtered.length" class="panel empty-state"><UsersRound :size="28" /> 未找到匹配成员</div>
        </div>
      </div>
    </div>
  </section>
