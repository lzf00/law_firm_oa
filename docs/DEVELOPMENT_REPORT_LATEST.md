...
- PostgreSQL 使用 Flyway V1–V14 连续生产迁移；生产就绪检查在全新数据库完成
  14/14 严格迁移，且组织和用户种子均为 0。开发演示环境另有 V100–V106，
  已在销毁全部隔离数据卷后完成 21/21 干净启动验证。
...
| 性能 | 1228.55 req/s；p50 9.83ms；p95 31.80ms；p99 73.13ms |
| 上传入口并发 | 20 并发启动通过 |
...
最新恢复演练使用 `backups/20260728T152358Z`：21 条迁移记录、4 个演示用户、4 份
测试文档、119 条审计记录恢复核对通过。该备份来自最终干净数据库验收环境，仅用于
本地验证，未加密，不能作为
正式生产备份。
...
7. 生产权限迁移早于开发组织和角色种子，导致全新演示管理员缺少后续权限；已增加
   最终开发角色/权限对账迁移，并在空数据卷上验证 31 项管理员权限。
