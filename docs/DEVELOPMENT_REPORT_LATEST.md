...
- PostgreSQL 使用 Flyway V1–V20 连续生产迁移；生产就绪检查在全新数据库完成
  20/20 严格迁移，且组织和用户种子均为 0。开发演示环境另有 V100–V110，
  已在全新隔离数据卷完成 31/31 干净启动验证。
...
| 验收项 | 最终结果 |
|---|---:|
| 后端单元/边界测试 | 117/117 通过 |
| JaCoCo 白盒覆盖 | 行 840/7391（11.36%）；分支 322/1852（17.39%） |
| 前端单元测试 | 12/12 通过 |
| API 真实依赖冒烟 | 47/47 通过 |
| 浏览器自动化 | 21/21 通过；7 个场景 × 中文桌面/英文桌面/中文移动端 |
| 可访问性与响应式 | 0 serious/critical，0 页面横向溢出 |
| 浏览器运行质量 | 0 控制台错误，0 HTTP 5xx |
| 100 用户负载 | 1200 请求，20 并发，失败 0 |
| 性能 | 586.78 req/s；p50 24.70ms；p95 71.25ms；p99 122.12ms |
| 上传入口并发 | 20 并发启动通过 |
...
| 回滚兼容 | V1–V20 连续、无 DROP/TRUNCATE/破坏性 NOT NULL |
...
JaCoCo 指令覆盖 13.17%、方法覆盖 17.25%。API、浏览器和负载测试不计入 JaCoCo，
...
7. 生产权限迁移早于开发组织和角色种子，导致全新演示管理员缺少后续权限；已增加
   最终开发角色/权限对账迁移，并在空数据卷上验证 49 项管理员权限。
